...
void my_BMP::_generateIP() {
    ...
    _initImageParameters(_bmpIP, "/rdc/gear.rdc");

    // load rd56 image data
    FlashFS rd56_f("/rdc/gear.rdc");
    rd56_f.read_f(bitmap, 0, 0, _bmpIP.frameNumber);
    ...
    _print_10x15(1, ip_part1, 55, 25, 7, 7, 7);
    _print_10x15(1, ip_part2, 55, 9, 7, 7, 7);

/*
    sprintf(text1, "%u", LEDcontrol.line_period);
    _print_10x15(1,text1, 55, 17, 7, 7, 7);
*/

    strcpy(text1,_ssid);
    _print_10x15(1, text1, 55, 72, 7, 7, 7);
    
    text1[0]=0x7E;              // Wifi symbol
    text1[1]=0x7F;
    text1[2]=0x00;
    _print_10x15(1, text1, 55, 88, 7, 7, 7);
...
void my_BMP::_generateAnalog() {
    ...
    _drawCircle(38, 7, 7, 7);
    _drawCircle(39, 7, 7, 7);
    for (int n=0; n<240; n=n+20) _drawRadius(n, 32, 39, 7, 7, 7);
    for (int n=0; n<240; n=n+4) _drawRadius(n, 35, 39, 7, 7, 7);
    char text[10]="12";
    _print_10x15(1, text, 55, 85, 7, 7, 7);
    strcpy(text,"6");
    _print_10x15(1, text, 55, 10, 7, 7, 7);
    strcpy(text,"3");
    _print_10x15(2, text, 100, 50, 7, 7, 7);
    strcpy(text,"9");
    _print_10x15(1, text, 15, 50, 7, 7, 7); 
  ...
  // hour hand
    ...
    _drawRadius(n, 0, 20, 7, 7, 7);

  // minute hand
    n = (int)((_myMin*60+_mySec)/15);
    _drawRadius(n, 0, 35, 7, 7, 7);
...
//  retrieve logoPath
  ...
  // load rd56 image data
  FlashFS rd56_f(wfPath);
  rd56_f.read_f(bitmap, 0, 0, _bmpWf.frameNumber);
}
...
void my_BMP::_generateDigital() {
        ...
        sprintf(text4,":");
        _print_12x18(1,text4,40,72, 7, 7, 7);
        _print_12x18(1,text4,70,72, 7, 7, 7);
        ...
        _print_12x18(1,text4,85,72, 7, 7, 7);
        sprintf(text4,"%02d.%02d.%04d",_myMday, _myMon+1, _myYear+1900);
        _print_10x15(1,text4,55,24, 7, 7, 7);
        ...
        _print_10x15(1,text4,55,7, 7, 7, 7);
}
...
void my_BMP::_generateLogoClock() {
    ...
    char text4[64];
    sprintf(text4, ":");
    _print_16x24(1,text4,55,70, 0, 0, 0);

    sprintf(text4,"%02d", _myHour);           
    _print_16x24(0,text4,15,70, 0, 0, 0);

    sprintf(text4,"%02d", _myMin);           
    _print_16x24(0,text4,59,70, 0, 0, 0);

    sprintf(text4,":%02d", _mySec);          
    _print_10x15(0,text4,70,48, 0, 0, 0);
}
...
//  retrieve logoPath
  ...
  // load rd56 image data
  FlashFS rd56_f(logoPath);
  rd56_f.read_f(bitmap, 0, 0, _bmpLogo.frameNumber);
}
...
FlashFS rd56_f(filePath);
rd56_f.read_f(bitmap, 0, 0, _bmpWeather.frameNumber);

    
  char text4[64];
  sprintf(text4, ":");
  _print_16x24(1,text4,55,70, 7, 7, 7);
  sprintf(text4,"%02d", _myHour);          
  _print_16x24(0,text4,15,70, 7, 7, 7);
  sprintf(text4,"%02d", _myMin);          
  _print_16x24(0,text4,59,70, 7, 7, 7);
 // sprintf(text4,":%02d", _mySec);
 // _print_10x15(0,text4,70,48, 7, 0, 0);
  ...
  _print_10x15(0,text4,2,50, 7, 7, 7);
  w_humi.toCharArray(text5, 20);
  sprintf(text4, "%s%%", text5);        
  _print_10x15(0,text4,78,50, 7, 7, 7);
...
void my_BMP::_generateImage() {
  ...
  // load rd56 image data
  FlashFS rd56_f(imagePath);
  rd56_f.read_f(bitmap, 0, 0, _bmpImage.frameNumber);
}
...
void my_BMP::_initImageParameters(ImageParameters& imageParameters, String imagePath) {
  if (imagePath != imageParameters.fileName) {
    ...
    FlashFS dummy{"/variables/dummy.txt"};
    
    File file = SD.open(imagePath.c_str(), FILE_READ);
...
void my_BMP::_print_10x15(int p_mode, char s[], int xpos, int ypos, int r, int g, int b) {                      // p_mode = print mode
  ...
  //      chr=(unsigned long)(chr_10x15[(int)s[j]-0x20][2*x]);       // 24 y bits
